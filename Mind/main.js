...
var // All Variable
  ...
  inpOptColor = document.querySelector(
    ".controls .opt-color-full .color-option .color-opt"
  ),
  btnResetColor = document.querySelector(
    ".controls .opt-color-full .reset-color"
  ),
  ...
  containerListItem = document.querySelector(
    ".controls .history ul .container-list"
  ),
  emptyHistory = document.querySelector(
    ".controls .history ul .msg-empty-history"
  ),
  ...
  btnCloseLogin = document.querySelector(
    ".wrapper-login .view-question-user .close-login"
  ),
  ...
  // Confirm
  btnConfirmLogin = document.querySelector(
    ".wrapper-login .view-question-user .wrapper-confirm .confirm"
  ),
  btnCancelLogin = document.querySelector(
    ".wrapper-login .view-question-user .wrapper-confirm .cancel"
  ),
  inputViewConfirm = document.querySelector(
    ".wrapper-login .view-question-user .wrapper-confirm .show-msg input"
  ),
  // End Var History
...
for (loopImgs = 0; loopImgs < 51; loopImgs++) {
  if (loopImgs <= 9) {
    imgs[loopImgs] =
      "https://khaled-mindness.netlify.app/imgs/imgsRandom/" +
      "0" +
      loopImgs +
      ".JPG";
  } else {
...
var newFuncToggleScale = function (e) {
  "use strict";
  toggleFullScreenGlobal(this);
  e.stopPropagation();
};
for (imgAboutCount = 0; imgAboutCount < imgAbout.length; imgAboutCount++) {
  ...
  wrapper.innerHTML = arrs
    .map(function (arr) {
      ...
      return (
        ...
        nameUser.dataset.name +
        ' <img src="' +
        viewPhotoUser.src +
        '"></div> <div class="view-history"> <div> <div><span>Status</span><span>' +
        arr.statusUser +
        "</span></div> <div><span>Level</span><span>" +
        arr.levelUser +
        "</span></div> </div> <div> <div><span>Score</span><span>" +
        arr.scoreUser +
        "</span></div> <div><span>Time</span><span>" +
        arr.timeUser +
        "</span></div> </div> </div> </li>"
      );
    })
...
wraViewPhotoUser.onmouseover = function () {
  "use strict";
  if (
    viewPhotoUser.getAttribute("src") ===
...
function funcLoopChangeLevel(numBoxs, numDelayGoker) {
  "use strict";
  var newFuncAddVisibile = function () {
      visibileImg(this, numBoxs, numDelayGoker);
    },
    imgGoker = document.querySelectorAll(
      'img[src="http://khaled-test-smart.bitballoon.com/imgs/imgsRandom/20.JPG"]'
    ),
    gok;
  for (x = 0; x < boxs.length; x++) {
